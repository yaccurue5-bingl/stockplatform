...
# KSIC 소분류(4자리) → 상위 업종 매핑 (우선 적용)
# 중분류로는 정확히 분류하기 어려운 케이스를 위한 세부 규칙
KSIC_DETAIL_RULES = {
    # 지주회사 (KSIC 64 금융업 내 별도 분리)
    "6420": "지주회사",
...
def get_top_industry(ksic_code: str) -> str:
    ...
    # 1. 소분류(4자리) 우선 확인 - 지주회사, 투자회사 등 특수 케이스
    if len(numeric_code) >= 4:
        detail_class = numeric_code[:4]
        if detail_class in KSIC_DETAIL_RULES:
            return KSIC_DETAIL_RULES[detail_class]
