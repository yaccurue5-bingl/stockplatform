...
type BuybackSubtype = 'CANCELLATION' | 'BUYBACK' | 'DISPOSAL'

/** headline + key_numbers 전체 텍스트로 자사주 이벤트 세부 분류 */
function classifyBuybackSubtype(
  headline: string | null,
  keyNums: string[],
): BuybackSubtype {
...
function findMetricValue(lines: string[], patterns: RegExp[]): string | null {
 ...
 * - publicKeyNums(상위 2개 공개 라인)에서만 수치 추출 → 잠금 우회 없음
 * - 분류(subtype)는 호출 전 전체 keyNums 기반으로 결정
 */
