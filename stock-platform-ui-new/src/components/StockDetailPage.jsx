...
const StockDetailPage = () => {
  ...
  useEffect(() => {
    const fetchDisclosures = async () => {
      try {
        setLoading(true);
        // 포인트: DB의 disclosure_insights 테이블에서 해당 종목코드(stock_code)로 데이터 조회
        ...
        if (!data || data.length === 0) {
          setError(`종목코드 ${ticker}에 대한 최신 공시 데이터가 없습니다. DB를 확인해주세요.`);
          return;
        }
  ...
  if (loading) return <div className="flex justify-center p-20 bg-[#0f172a] min-h-screen"><Loader2 className="animate-spin text-blue-500" /></div>;
  if (error) return (
    <div className="p-10 text-center bg-[#0f172a] min-h-screen">
      <AlertCircle className="mx-auto text-red-500 mb-4" />
      <p className="text-slate-400">{error}</p>
      <button onClick={() => navigate('/')} className="mt-4 text-blue-500 hover:underline">홈으로 돌아가기</button>
    </div>
  );
    ...
    <div className="min-h-screen bg-[#0f172a] text-slate-200 p-6">
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        <div className="mb-8 border-b border-slate-800 pb-6">
          <h1 className="text-3xl font-bold text-white tracking-tight">{disclosures[0]?.corp_name}</h1>
          <p className="text-slate-500 mt-1">Intelligence Report: Recent 5 Critical Filings</p>
        </div>
            ...
            <div key={item.id} className="bg-slate-900 border border-slate-800 p-6 rounded-2xl hover:border-slate-700 transition">
              ...
              <div className="bg-[#0f172a] p-4 rounded-xl border-l-4 border-blue-500">
                <div className="flex gap-2 text-blue-400 mb-2 font-bold text-xs uppercase tracking-wider">
                  <MessageSquare size={14}/> AI Insight Summary
                </div>
